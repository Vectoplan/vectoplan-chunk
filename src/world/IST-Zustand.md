...
Stand: 2026-07-17  
Status: PostgreSQL-gestützter, projektgescopter Chunk-Service mit strikt getrenntem Runtime-/DB-Bootstrap, bestätigter Project-Access-Persistenz, vollständiger Rollen-/Gruppen-API, discovery-kompatiblem Earth-Provider und atomar bestätigter Flat-/Earth-Provisionierung.

Diese Aktualisierung **kürzt die bisherige IST-Dokumentation nicht**. Alle historischen und fachlichen Detailabschnitte bleiben erhalten. Aussagen aus älteren Zwischenständen werden nicht gelöscht, sondern durch den neuen maßgeblichen Stand vom 17. Juli 2026 ergänzt und, wo nötig, ausdrücklich als historisch oder noch offen eingeordnet.

Der aktuell bestätigte Gesamtslice umfasst:

```text
Runtime und Bootstrap
→ normaler Gunicorn-Start bleibt read-only
→ Tabellen, Seeds und Reparaturen laufen ausschließlich im expliziten Init-/Bootstrap-Pfad
→ Container-Neubau und Recreate erfolgreich
→ vectoplan-chunk healthy
→ aktueller lokaler HTTP-Port = 5102
→ aktueller lokaler PostgreSQL-Port = 15433

Projektgraph
→ Project
→ Universe
→ konkrete WorldInstance world_spawn
→ Flat bleibt Default-Provider
→ Earth kann explizit pro App-Projekt provisioniert werden
...
→ Defaultrollen owner/admin/editor/viewer
→ Owner-Zuweisung für externe User-ID "1"
→ keine Cross-Service-Foreign-Keys zu Auth oder App
→ authzEnforced = false

Earth providerweit
→ Earth-Provider erfüllt den neutralen Discovery-Vertrag
→ Discovery erkennt flat und earth als gültig
→ WorldLoader und WorldService können Earth über den neutralen Adapter laden
→ Provider-Version = 1.1.1
→ Air-only-GeneratedChunk mit 4096 Zellen
→ requested und canonical Chunkadresse bleiben getrennt

Earth projektgebunden
→ kompakte Earth-Referenz wird zu GlobalReferencePoint normalisiert
→ CRS EPSG:4979 wird kanonisch als WKT2:2019 persistiert
→ Earth-v1-Grid wird ergänzt
→ Referenzfingerprint wird als Domain-Fingerprint gespeichert
→ WorldInstance world_spawn wird mit provider/template earth erzeugt
→ Earth-Grenzen -1024 bis 8192 werden korrekt persistiert
→ präziser Spawn und Integer-Spawn sind konsistent
→ erster identischer Wiederholungsrequest ist idempotent
→ bestehender fehlerhafter Earth-Zustand wird einmalig repariert
→ zweiter Reparaturrequest bleibt unverändert
```

Wichtige aktuelle Abgrenzung:

```text
Bestätigt
→ Earth-Discovery
→ Earth-WorldService-Fallback
→ Earth-Debugpfad
→ kanonische Earth-Provisionierung
→ Project Access Bootstrap
→ PostgreSQL-Transaktion und Idempotenz

Noch offen
→ produktiver projektgebundener Earth-Snapshot-/Commandpfad über die periodische X-Naht
→ vollständiger Bootstrap-/Chunk-Retest des zuletzt reparierten Earth-Testprojekts
→ world_test.py-Limit für eine vollständige X-Periode
→ Autorisierungsdurchsetzung auf Basis der gespeicherten Access-Daten
```

Aktuell maßgebliche Teil-Dokumentationen:

```text
services/vectoplan-chunk/models/IST-Zustand.md
→ 14 persistente Modelklassen einschließlich Project Access

services/vectoplan-chunk/routes/IST-Zustand.md
→ zehn Route-Dateien, Project-Access-Blueprint und atomare Provisionierung

services/vectoplan-chunk/src/world/IST-Zustand.md
→ neutraler Flat-/Earth-Providervertrag und bestätigter Earth-Adapter

services/vectoplan-chunk/IST-Zustand.md
→ serviceweite Zusammenführung und historische Entwicklung
```

---

## Dokumentierter Zwischenstand vom 13. Juli 2026
...
## 28. Aktualisierung 2026-07-17 – Project Access, neutraler Earth-Adapter und atomare Earth-Provisionierung

Dieser Abschnitt ist der aktuelle serviceweite Ergänzungsstand nach:

```text
Einführung der Project-Access-Models
→ ProjectRole
→ ProjectGroup
→ ProjectGroupMember
→ ProjectRoleAssignment

Einführung des Project-Access-Service
→ Defaultrollen synchronisieren
→ Owner-Zuweisung sicherstellen
→ Gruppen und Mitgliedschaften verwalten
→ User- und Gruppenrollen zuweisen

Einführung der Project-Access-Routen
→ produktiver Blueprint
→ Status-/Contract-Endpunkte
→ Rollen-, Gruppen-, Membership- und Assignment-Endpunkte

Earth-Provider-Adapter
→ neutraler Discovery-/Loader-/WorldService-Vertrag
→ import-sichere Lazy-Auflösung
→ GeneratedChunk-Adapter
→ requested-/canonical-Adressmetadaten

Earth-Provisioning
→ gezielte PostgreSQL-Row-Locks
→ kanonische GlobalReferencePoint-Erzeugung
→ Earth-spezifische Weltgrenzen
→ referenzbasierter Spawn
→ Access-Bootstrap in derselben Transaktion
→ bestätigte Reparatur und Idempotenz
```

Die bisherigen Flat-, Systemblock-, Snapshot-, Event-, Command-, Bootstrap- und App-/Editor-Aussagen bleiben gültig, soweit sie nicht in diesem Abschnitt ausdrücklich präzisiert werden.

### 28.1 Aktueller Runtime- und Containerzustand
...
Bestätigter Zustand nach Healthcheck:
...
Die älteren historischen Befehle mit Port `5002`, Port `5433` oder direkt angenommenen Container-Namen bleiben als damaliger Testnachweis erhalten. Für den aktuellen Compose-Stand gelten jedoch:
...
Der Runtime-Startup bleibt unverändert read-only:

```text
keine Tabellenanlage
keine Migration
kein Default-Seeding
keine Seed-Reparatur
keine Chunkmaterialisierung
keine Snapshotladung
keine Eventladung
keine Commandladung
keine ObjectRef-Ladung
```

### 28.2 Aktuell bestätigte Blueprint- und Routeoberfläche

Beim bestätigten Startup wurden registriert:
...
world_test
earth_debug
→ optionale Entwicklungs-/Diagnose-Blueprints

editor
→ optionaler Legacy-Blueprint
```

Aktuell dokumentierter Routenstand:

```text
zehn Python-Dateien im Routeordner
81 explizit deklarierte HTTP-Routen
83 Flask-URL-Regeln im bestätigten App-Startup
```

Die Differenz entsteht durch Flask-/App-Regeln außerhalb der expliziten Route-Deklarationen, beispielsweise Root- oder Static-Regeln.
...
Aktuelle wichtige Modulversionen:
...
Der Blueprint `project_access` ist Teil der produktiven Routeoberfläche. Ein Import- oder Registrierungsfehler dieses Pflichtmoduls darf den produktiven Startup nicht stillschweigend als bereit melden.

### 28.3 Project-Access-Architektur

Neue persistente Struktur:
...
Externe Identitätsregel:

```text
User-ID
→ opaque String
→ zum Beispiel "1"
→ kein Foreign Key zur Auth-Datenbank
→ kein Foreign Key zur App-Datenbank
```

Gruppen besitzen eine interne projektgescopte Gruppen-ID. Mitgliedschaften verknüpfen diese Gruppe mit externen User-ID-Strings.

Rollenzuweisungen unterstützen:
...
Die Access-Models speichern den Vertrag. Sie entscheiden noch nicht selbst, ob ein HTTP-Request ausgeführt werden darf.

Harte aktuelle Einordnung:
...
```text
Rollen und Zuweisungen sind persistiert.
Access-Readiness ist prüfbar.
Owner und Defaultrollen werden provisioniert.

Aber:
Die produktiven Projekt-, Chunk- und Commandrouten erzwingen diese Rollen
noch nicht als Authorizer vor jedem Zugriff.
```

### 28.4 Standardrollen und Berechtigungen

Beim ersten Projekt-Access-Bootstrap werden vier Systemrollen sichergestellt:
...
```text
Ein neu provisioniertes Projekt erhält eine aktive Owner-Rollenzuweisung.
Standard-Owner-User-ID im aktuellen Übergangsvertrag = "1".
```

Die Owner-Zuweisung besitzt:
...
```text
Project Access initialisieren
→ Defaultrollen erstellen oder synchronisieren
→ gelöschte Systemrollen bei Bedarf wiederherstellen
→ Owner-Zuweisung sicherstellen
→ Änderungen und Wiederverwendung zählen
→ strukturiertes Ergebnis zurückgeben
```

Der Service führt keinen unabhängigen äußeren Commit aus, wenn er im Provisioning-Transaktionspfad verwendet wird.

Aktuelle Ergebnisgruppen:
...
Beim bestätigten ersten Earth-Provisioning wurden erzeugt:
...
Der Route-Slice umfasst 26 Endpunkte in folgenden Gruppen:

```text
Status und Contract
→ Project-Access-Readiness
→ öffentliche Vertragsbeschreibung
...
→ Rolle erstellen
→ Rolle ändern
→ Rolle löschen beziehungsweise deaktivieren
→ Defaultrollen synchronisieren
...
→ Gruppe erstellen
→ Gruppe ändern
→ Gruppe löschen beziehungsweise deaktivieren
...
→ direkte User-Zuweisung
→ Gruppen-Zuweisung
→ Zuweisung ändern
→ Zuweisung widerrufen oder löschen
```

Der bestätigte Status meldete:
...
`databaseQueried=false` im kompakten Status bedeutet, dass der Vertrag ohne unnötige tiefe Datenabfrage diagnostiziert werden kann. Es bedeutet nicht, dass die Tabellen fehlen.

### 28.7 Erweiterte Modelregistrierung

Die Modelschicht besitzt nun 14 persistente Klassen:
...
`models/__init__.py` registriert zusätzlich:

```text
project_access.py
```

Die Modeldiagnose kennt:
...
kritische Spalten
Relationships
Project-Access-Contract-Readiness
```

Der Project-Access-Zweig ändert die bestehende Welt-, Snapshot-, Event- und Objektstruktur nicht. Er ergänzt die projektgescopte Zugriffsdatenebene parallel zum Universe-/World-Graphen.

### 28.8 Neutraler Earth-Provideradapter
...
Aktuelle Provider-Version:

```text
1.1.1
```

Die Datei besitzt weiterhin den WorldInstance-spezifischen Kern:
...
Zusätzlich implementiert sie jetzt den neutralen Providervertrag:

```python
get_provider_info()
get_default_config_path()
load_world_config(config_path=None)
validate_world_config(raw_config)
create_world_definition(raw_config)
generate_chunk(world, request)
```

Optionale Diagnosefunktionen:
...
Import-Sicherheitsregel:

```text
Neutraler Import des Provider-Moduls
→ lädt nicht unnötig die komplette Coordinates-/Georeferencing-Kette
→ verwendet defensive Lazy-Auflösung
→ bleibt für Discovery importierbar
```

Damit ist der frühere Fehler:

```text
ImportError: attempted relative import beyond top-level package
```

beseitigt. Die Ursache war zusätzlich durch eine versehentlich am falschen Pfad abgelegte Datei sichtbar geworden; nach Wiederherstellung der korrekten Route- und Providerdateien startete der Service stabil.

### 28.9 Bestätigte World-Discovery

Bestätigtes Discovery-Ergebnis:
...
Chunkgenerierung unterstützt
keine fehlenden Pflichtfunktionen
keine Errors
keine Warnings
```

Die Discovery prüfte für Earth erfolgreich:
...
### 28.10 Neutrale Earth-WorldDefinition

Der providerweite Adapter erzeugt eine neutrale `WorldDefinition` mit:
...
worldWidthCells   = 40.000.000
worldHeightCells  = 20.000.000
worldWidthChunks  = 2.500.000
worldHeightChunks = 1.250.000
```
...
Der neutrale Adapter verwendet eine nicht persistierte Referenz für Discovery und World-Test:
...
Diese Referenz ist ausschließlich Adapter-/Diagnosekontext. Sie ersetzt niemals den persistierten `GlobalReferencePoint` einer konkreten Projekt-WorldInstance.

### 28.11 Neutraler Earth-Chunk

Bestätigter Aufruf:

```text
GET /world-test/api/worlds/earth/chunks?chunkX=0&chunkY=0&chunkZ=0
```
...
Der neutrale `GeneratedChunk` besitzt eine Kompatibilitätspalette, verwendet in der aktuellen Air-only-Basis aber ausschließlich:
...
```text
requested Chunkadresse
→ darf für Response und Diagnose sichtbar bleiben

canonical Chunkadresse
→ ist die einzige zulässige physische Persistenzadresse
```

Ein Content-Hash-Vergleich zwischen Alias und kanonischer Anfrage ist nicht allein maßgeblich, weil Hashverträge je nach Schicht auch angeforderte Koordinaten enthalten können. Maßgeblich sind Kanonisierungsmetadaten und die physische Storageadresse.

### 28.12 Flat-Regression nach Earth-Adapter

Bestätigter Flat-Chunk:
...
```text
16 × 16 = 256
→ genau eine Terrainlage bei worldY = 0
```

Der Earth-Adapter hat den bestehenden Flat-Generatorpfad damit nicht beschädigt.

### 28.13 Atomare Projektprovisionierung
...
```text
PROVISIONING_SERVICE_VERSION = 2.0.3
```

Zusätzliche Policy-Versionen:

```text
project-provisioning-row-lock.v2
project-provisioning-earth-reference.v2
project-provisioning-world-state.v2
```

Der Provisioning-Pfad erzeugt beziehungsweise synchronisiert in einer Transaktion:
...
```text
commit = true
→ Provisioning besitzt Commit und Rollback

commit = false
→ Aufrufer besitzt Commit und Rollback
```
...
seedet keine globalen Blockdaten
lädt keine Chunks
schreibt keine Snapshots
schreibt keine Events
schreibt keine Commands
ruft vectoplan-app nicht zurück
```

### 28.14 PostgreSQL-Row-Lock-Korrektur
...
```text
SQLAlchemy-Relationship mit lazy="joined"
→ impliziter LEFT OUTER JOIN
→ query.with_for_update()
→ PostgreSQL versucht auch nullable Join-Seite zu sperren
```
...
query.with_for_update(of=model)
```

Resultierende Semantik:

```sql
SELECT ...
FROM universes
WHERE ...
FOR UPDATE OF universes
```

Der zentrale Fix gilt für:
...
Fallbacks bleiben vorhanden für:

```text
ältere SQLAlchemy-ähnliche Wrapper
Test-Doubles ohne of-Keyword
Queries ohne Lock-API
```

Die produktive PostgreSQL-Ausführung bestätigte, dass der frühere Outer-Join-Lockfehler nicht mehr auftritt.

### 28.15 Kompakte Earth-Referenz als API-Eingabe

Unterstützte kompakte Eingabe:
...
```text
HTTP-Eingabe
→ numerische Werte als Decimal normalisieren
→ coordinateOrder validieren
→ X/Y bei latitude-first-Eingabe kontrolliert tauschen
→ GlobalCoordinate erzeugen
→ CRS über resolve_crs() auflösen
→ vollständige WKT2:2019-CrsDefinition erzeugen
→ Earth-v1-Griddefinition laden
→ GlobalReferencePoint erzeugen
→ to_persistence_dict()
→ Domain-Fingerprint verwenden
```
...
```text
CRS ist Pflicht.
CRS wird nicht aus Zahlenwerten geraten.
alwaysXY muss true sein.
Koordinate besitzt zwei oder drei Werte.
Earth-v1-Grid darf nicht stillschweigend ersetzt werden.
```

Vollständige kanonische Referenzpayloads mit:
...
### 28.16 Kanonischer Referenzvertrag

Bestätigte persistierte Struktur:

```text
schemaVersion   = earth-global-reference.schema.v1
referenceVersion = 1
fingerprint     = Domain-SHA-256
```
...
gridId           = vectoplan-earth-grid
gridVersion      = 1
gridKey          = vectoplan-earth-grid@1
projectionId     = vectoplan-periodic-equirectangular
projectionVersion = 1
topologyType     = periodic-x-v1
axisConvention   = x-east-y-up-z-north
...
Bestätigter Fingerprint des Testreferenzpunkts:

```text
38747762c2846401d9bc970ea59a872d7aeff090e0992565d8a9d8eea3c850a3
```

Dieser Fingerprint war identisch in:
...
### 28.17 Earth-spezifischer World-State-Vertrag
...
VECTOPLAN_CHUNK_DEFAULT_SPAWN_*
```

wurde versehentlich auch auf Earth angewendet.

Korrigierte Namensräume:

```text
Flat
→ VECTOPLAN_CHUNK_DEFAULT_*

Earth
→ VECTOPLAN_CHUNK_EARTH_*
```
...
Unterstützte Earth-spezifische Overrides:
...
Der von `WorldInstance.create_earth_spawn()` berechnete präzise Spawn wird nicht mehr durch Flat-/Generic-Defaults überschrieben.

Integer-Ableitung:

```text
spawn_x = floor(spawn_x_precise)
spawn_y = floor(spawn_y_precise)
spawn_z = floor(spawn_z_precise)
```

Floor-Semantik gilt auch für negative Werte:
...
### 28.18 Bestätigte Earth-Provisionierung

Verwendetes Beispielprojekt:

```text
external App project = prj-earth-smoke-20260717173207
Chunk project        = chk_prj_earth_20260717173207
Universe             = chk_uni_earth_20260717173207
World                = world_spawn
```
...
created        = true
updated        = false
serviceVersion = 2.0.2 zum Erstellungszeitpunkt
worldTemplate  = earth
```
...
### 28.19 Bestätigte Reparatur bestehender Earth-Werte

Nach Version 2.0.3 wurde dasselbe bereits bestehende Projekt erneut provisioniert.

Erster Reparaturrequest:
...
```text
spawn_x_precise = 4.44444444444444500000
spawn_y_precise = 13.00000000000000000000
spawn_z_precise = 3.55555555555555540000
```
...
```text
projectDbId       = 6
projectId         = chk_prj_earth_20260717173207
accessInitialized = true
```

Unmittelbarer zweiter identischer Request:
...
```text
bestehender Drift
→ genau einmal repariert

identischer Folgerequest
→ keine Revisionserhöhung ohne Änderung
→ keine doppelten Rollen
→ keine doppelte Owner-Zuweisung
→ kein doppeltes Universe
→ keine doppelte WorldInstance
```

### 28.20 Fehler- und Rollbackverhalten

Der frühere Factoryfehler bei unvollständiger Earth-Referenz führte zu:

```text
Project erzeugt
Universe-Erzeugung begonnen
World-Factory scheitert
→ vollständiger Rollback
```

Bestätigt war danach:

```text
kein halbfertiges Project
kein halbfertiges Universe
keine halbfertige WorldInstance
keine unvollständige Access-Struktur
```

PostgreSQL-Sequenzen werden durch Rollback nicht zurückgesetzt. Lücken in internen numerischen IDs sind deshalb erwartbar und kein Persistenzfehler.

Der aktuelle Fehlervertrag übernimmt zusätzlich sichere Domaininformationen:
...
Domain-Details
HTTP-Status
retryable, falls vorhanden
```

Vollständige Tracebacks oder unkontrollierte lange CRS-Payloads werden nicht als öffentliche Fehlerdetails ausgegeben.

### 28.21 Aktuelle Provisioning-Antwort

Eine erfolgreiche Provisionierungsantwort enthält:
...
Die Antwort ist bewusst ausführlich und eignet sich für Debugging und Integrationsabgleich.

Später sinnvoller optionaler Kompaktmodus:

```text
includeAccessDetails=false
includeCrsDefinition=false
includeMetadata=false
```

Dieser Kompaktmodus ist aktuell nicht implementiert und kein Blocker für den bestätigten Pfad.

### 28.22 Aktuell bestätigte Readiness

Kompakter Project-/Bootstrap-Status:
...
seedReady                 = true
defaultWorldReady         = true
projectAccessSchemaReady  = true
defaultProjectAccessReady = true
```
...
```text
GlobalReferencePoint valid
CRS valid
EarthGridFrame valid
global/local Roundtrip valid
Spawn valid
Chunk valid
```

### 28.23 Bekannter verbleibender `world_test.py`-Fehler

Datei:

```text
services/vectoplan-chunk/routes/world_test.py
```

Aktuelle feste Grenze:

```python
MAX_ABS_CHUNK_COORDINATE = 1_000_000
```

Earth besitzt jedoch:
...
wird deshalb vor dem Provider abgewiesen.

Aktuelle fehlerhafte Klassifizierung:

```text
HTTP 500
code = unexpected_world_error
```

Erwartete Zielklassifizierung:
...
```text
Der vollständige Periodic-X-Wrap ist über die Debugroute noch nicht bestätigt.
Der Provider wurde durch diesen Test nicht widerlegt, weil die Anfrage ihn nicht erreicht.
```

Die nächste Quelltextkorrektur nach Abschluss der Dokumentationsaktualisierung sollte daher in `routes/world_test.py` erfolgen.

### 28.24 Projektgebundene Earth-Integration – bestätigt und offen

Bestätigt:

```text
WorldInstance mit provider/template earth wird persistiert.
GlobalReferencePoint wird vollständig persistiert.
Earth-Grenzen werden korrekt persistiert.
Spawn wird referenzbasiert persistiert.
Project Access wird atomar initialisiert.
Provisioning ist idempotent.
```

Noch nicht nach dem letzten Fix vollständig erneut bestätigt:

```text
GET /projects/<chunkProjectId>/bootstrap?includeAccess=true
→ vollständige aktuelle Earth-Antwort

GET /projects/<chunkProjectId>/worlds/world_spawn/chunks?chunkX=0&chunkY=0&chunkZ=0
→ instance-spezifischer Earth-Provider aus DB-Referenz

Snapshotmaterialisierung einer Earth-Welt
Commandausführung einer Earth-Welt
periodische Alias-Deduplizierung im Snapshotpfad
Dirty-Chunks über die Earth-X-Naht
automatischer Referenz-Lock nach erster Materialisierung
```

Diese Punkte bleiben ausdrücklich offen, obwohl die Provisionierung selbst bestätigt ist.

### 28.25 Autorisierung – aktuelle Grenze

Persistiert und routbar:
...
Mitgliedschaften
Zuweisungen
Permission-Allow-/Deny-Verträge
Owner-Zuweisung
```
...
```text
Project-Lesen nur bei view
Project-Ändern nur bei edit/manage
Project-Löschen nur bei delete
Teamverwaltung nur bei manage_team
Transfer nur bei transfer
Chunk-/Commandzugriff nach effektiver Rolle
```

Die dauerhafte Sicherheitsregel lautet:

```text
Access-Daten sind nicht gleich Autorisierung.
Erst ein zentraler Authorizer macht aus gespeicherten Rollen eine Zugriffskontrolle.
```

Bis dahin müssen Status- und Responseverträge weiterhin offen melden:

```text
authzEnforced = false
```

### 28.26 Aktualisierte serviceweite Ordnerstruktur
...
### 28.27 Aktualisierte Invarianten 131 bis 174

Zusätzlich zu den bisherigen Invarianten 1 bis 130 gelten nun:

```text
131. Project Access ist projektgescopt.
132. Externe User-IDs bleiben opaque Strings.
133. Externe User-IDs besitzen keine Foreign Keys zu anderen Services.
134. ProjectRole gehört genau zu einem Chunk-Projekt.
135. ProjectGroup gehört genau zu einem Chunk-Projekt.
136. ProjectGroupMember verbindet eine Projektgruppe mit einer externen User-ID.
137. ProjectRoleAssignment adressiert genau ein User- oder Gruppensubjekt.
138. owner, admin, editor und viewer sind kanonische Systemrollen.
139. Nur owner besitzt standardmäßig transfer.
140. admin besitzt einen ausdrücklichen transfer-Deny.
141. Ein neues Projekt erhält eine aktive Owner-Zuweisung.
142. Der aktuelle Übergangsowner ist externe User-ID "1".
143. Access-Models führen keinen HTTP-Authorizer aus.
144. authzEnforced bleibt false, bis ein zentraler Authorizer integriert ist.
145. Project Access wird mit dem Projektgraphen atomar provisioniert.
146. Ein Rollback des Projektgraphen rollt auch neue Access-Zeilen zurück.
147. Project-Access-Status darf ohne tiefe Datenabfrage diagnostizierbar sein.
148. Der Earth-Provider erfüllt den neutralen Discovery-Pflichtvertrag.
149. Discovery erkennt flat und earth als gültige Provider.
150. Der neutrale Earth-Adapter verwendet keine persistierte Projekt-Referenz.
151. Der neutrale Earth-Adapter ersetzt nicht den WorldInstance-spezifischen Provider.
152. Earth-Adapterchunks bleiben Air-only.
153. requested und canonical Chunkadressen müssen getrennt bleiben.
154. Nur die kanonische Adresse darf physisch persistiert werden.
155. Die Earth-Weltbreite beträgt 2.500.000 Chunks.
156. Eine Debuggrenze von 1.000.000 Chunks reicht nicht für eine volle X-Periode.
157. Ungültige Querykoordinaten sind Clientfehler und kein HTTP-500-Systemfehler.
158. Provisioning-Row-Locks müssen auf die Basistabelle begrenzt werden.
159. Lock-Lookups dürfen keine unnötigen eager Outer Joins enthalten.
160. Kompakte Earth-Eingaben müssen vor der World-Factory kanonisiert werden.
161. GlobalReferencePoint ist die kanonische Earth-Persistenzwahrheit.
162. Der Referenzfingerprint stammt aus dem Domainobjekt.
163. EPSG:4979 wird kanonisch als WKT2:2019 persistiert.
164. Earth-v1 verwendet das Grid vectoplan-earth-grid@1.
165. Flat-Default-ENV darf Earth-Grenzen nicht überschreiben.
166. Earth verwendet standardmäßig min_y=-1024 und max_y=8192.
167. Der Earth-Integer-Spawn wird aus dem präzisen Spawn gefloort.
168. Negative präzise Spawnwerte verwenden mathematische Floor-Semantik.
169. access.projectDbId muss in der Provisionierungsantwort konsistent gesetzt sein.
170. Ein Reparaturrequest darf bestehenden Drift genau einmal korrigieren.
171. Der unmittelbar folgende identische Request muss unverändert idempotent sein.
172. PostgreSQL-Sequenzlücken nach Rollback sind kein Datenkonsistenzfehler.
173. Eine erfolgreiche Provisionierung bestätigt noch nicht automatisch den Earth-Snapshot-/Commandpfad.
174. Flat bleibt Default, solange Earth nicht explizit gewählt wird.
```

### 28.28 Aktuelle PowerShell-Smoke-Tests
...
$ProjectStatus = Invoke-RestMethod `
  -Method Get `
  -Uri "$BASE/projects/_status?checkDatabase=true&includeCounts=false"
...
Invoke-RestMethod `
  -Method Get `
  -Uri "$BASE/world-test/api/worlds?forceRefresh=true" |
  ConvertTo-Json -Depth 30
```
...
Invoke-RestMethod `
  -Method Get `
  -Uri "$BASE/world-test/api/worlds/earth/chunks?chunkX=0&chunkY=0&chunkZ=0" |
  ConvertTo-Json -Depth 30
```

### 28.29 Aktueller Earth-Provisioning-Smoke-Test

```powershell
$BASE = "http://localhost:5102"
$Suffix = Get-Date -Format "yyyyMMddHHmmss"

$AppProjectId = "prj-earth-smoke-$Suffix"
$ChunkProjectId = "chk_prj_earth_$Suffix"
$ChunkUniverseId = "chk_uni_earth_$Suffix"

$EarthProjectBody = @{
    chunkProjectId = $ChunkProjectId
    chunkUniverseId = $ChunkUniverseId
    chunkWorldId = "world_spawn"
    name = "Earth Smoke Test $Suffix"
    description = "Earth-Provisionierungs- und Chunk-Test"
    ownerUserId = "1"
    actorUserId = "1"
    worldTemplate = "earth"
    earthReference = @{
...
$ProvisionUri = "$BASE/projects/by-app/$AppProjectId"
[uri]$ProvisionUri

$EarthProvision = Invoke-RestMethod `
  -Method Put `
  -Uri $ProvisionUri `
  -ContentType "application/json" `
  -Body $EarthProjectBody

$EarthProvision |
  Select-Object ok, code, created, updated, serviceVersion, worldTemplate |
  Format-List
```

Erwartung beim ersten Request:
...
```powershell
$EarthProvision.access |
  Select-Object projectDbId, projectId, accessInitialized |
  Format-List
```
...
```powershell
$EarthProvisionAgain = Invoke-RestMethod `
  -Method Put `
  -Uri $ProvisionUri `
  -ContentType "application/json" `
  -Body $EarthProjectBody
...
$EarthProvisionAgain |
  Select-Object ok, code, created, updated |
  Format-List
```
...
### 28.30 Aktualisierte Prioritäten
...
```text
providerabhängige Chunkgrenzen
volle Earth-X-Periode testbar
ungültige Koordinaten als HTTP 400
stabile WorldError-Serialisierung
```
...
```text
neue neutrale Providerfunktionen über die Paketfassade exportieren
Import-Sicherheit bewahren
Providerstatus und Cache-Reset vollständig synchronisieren
```
...
```text
persistierten GlobalReferencePoint laden
instance-spezifischen EarthWorldProvider erzeugen
vor Snapshotlookup kanonisieren
nur kanonische Adressen materialisieren
```
...
```text
Earth-Blockposition kanonisieren
Dirty-Chunks über X-Naht berechnen
Alias-Commands deduplizieren
Referenz nach erster Materialisierung sperren
Project-Access-Authorizer später zentral vorschalten
```

### 28.31 Aktualisierter Gesamtbefund

Der aktuelle serviceweite Stand lautet:

```text
Der Chunk-Service startet stabil und read-only.
Der explizite DB-Bootstrap bleibt getrennt.
Schema, Seeds und Defaultgraph sind bereit.
Project Access ist persistiert und routbar.
Defaultrollen und Owner-Zuweisung werden atomar erzeugt.
Autorisierung wird noch nicht erzwungen.

Flat bleibt der bestätigte Defaultprovider.
Der Flat-Generator-, Snapshot- und Command-Slice bleibt funktionsfähig.

Earth ist nicht mehr nur ein isolierter Debugkern.
Der Provider ist discovery-, loader- und WorldService-kompatibel.
Die projektgebundene Earth-Provisionierung ist bestätigt.
Der GlobalReferencePoint wird kanonisch persistiert.
PostgreSQL-Locks sind Outer-Join-sicher.
Earth-Grenzen und Spawn sind konsistent.
Provisionierung und Reparatur sind idempotent.

Noch offen ist die vollständige projektgebundene Earth-Read-/Write-Kette:
Snapshotlookup, Materialisierung, Commands, Events und Dirty-Chunks über die Naht.
```

Dauerhafte Architekturregel:

```text
models
→ persistente Verträge und lokale Invarianten

services
→ fachliche Orchestrierung und Transaktionen

routes
→ HTTP, Contractvalidierung und Response

provider
→ deterministische Weltbasis ohne Datenbanktransaktion

Project Access
→ gespeicherter Zugriffsvertrag

Authorizer
→ noch zu ergänzende Durchsetzung des Zugriffsvertrags
```

---

## 29. Dokumentations- und Verifikationsnachweis vom 17. Juli 2026

Für diese Aktualisierung wurden die folgenden bereits aktualisierten Teilbestandsaufnahmen zusammengeführt:
...
Zusätzlich berücksichtigt wurden die bestätigten Laufzeitergebnisse für:

```text
Docker-Build und Container-Health
Blueprintregistrierung
Project-Access-Status
Earth-Discovery
Earth-Debug
Earth-WorldService-Chunk
Flat-Regression
PostgreSQL-Row-Lock-Fix
Earth-Referenznormalisierung
Earth-Provisionierung
Earth-Reparatur
Access.projectDbId
Provisionierungs-Idempotenz
```

Die historischen Abschnitte dieser Datei wurden bewusst nicht entfernt. Der aktuelle Stand ist über:
...
maßgeblich eingeordnet.

Noch nicht als erfolgreich behauptet werden:

```text
vollständiger Periodic-X-HTTP-Test über world_test.py
projektgebundener Earth-Chunk nach dem letzten Provisioning-Fix
Earth-Snapshotmaterialisierung
Earth-Commands über die Naht
Authorizer-Durchsetzung
```

Damit bleibt die Dokumentation überprüfbar und trennt klar zwischen:
